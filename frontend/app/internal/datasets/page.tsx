...
type Question = {
  ...
  is_approved: boolean; deleted_at: string | null;
};
...
export default function DatasetsPage() {
  ...
  const [approvedPool,setApprovedPool]= useState<{ question: Question; datasetName: string }[]>([]);
  ...
  // Stats per dataset
  const [stats, setStats] = useState<Record<string, { questions: number; approved: number }>>({});
  ...
  function loadDatasets() {
    fetch(`${API}/internal/datasets`)
      ...
      .then((d: Dataset[]) => {
        ...
        // Load stats for each
        d.forEach((ds) => {
          fetch(`${API}/internal/datasets/${ds.id}/questions?include_deleted=false`)
            .then((r) => r.json())
            .then((qs: Question[]) => {
              setStats((prev) => ({
                ...prev,
                [ds.id]: {
                  questions: qs.length,
                  approved:  qs.filter((q) => q.is_approved).length,
                },
              }));
            })
            .catch(() => {});
  ...
  async function createDataset(e: React.FormEvent) {
    ...
    setStats((prev) => ({ ...prev, [d.id]: { questions: 0, approved: 0 } }));
  ...
  async function removeQuestion(qid: string) {
    ...
    if (activeId) setStats((prev) => ({ ...prev, [activeId]: { ...prev[activeId], questions: (prev[activeId]?.questions ?? 1) - 1 } }));
  }

  // Import từ approved pool — load tất cả câu approved từ mọi dataset
  function openImport() {
    setSrcSelected(new Set()); setSrcSearch(""); setSrcDataset("");
    setShowImport(true);
    // Load approved questions từ tất cả datasets (trừ dataset hiện tại)
    Promise.all(
      datasets
        .filter((d) => d.id !== activeId)
        .map((d) =>
          fetch(`${API}/internal/datasets/${d.id}/questions?include_deleted=false`)
            .then((r) => r.json())
            .then((qs: Question[]) =>
              qs.filter((q) => q.is_approved).map((q) => ({ question: q, datasetName: d.name }))
            )
            .catch(() => [] as { question: Question; datasetName: string }[])
        )
    ).then((results) => setApprovedPool(results.flat()));
  }

  async function doImport() {
    if (!activeId || srcSelected.size === 0) return;
    setImporting(true);
    const selected = approvedPool
      .filter((p) => srcSelected.has(p.question.id))
      .map((p) => p.question);
    await fetch(`${API}/internal/questions/batch`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify(selected.map((q) => ({
        dataset_id: activeId,
    ...
    setStats((prev) => ({
      ...prev,
      [activeId]: { ...prev[activeId], questions: (prev[activeId]?.questions ?? 0) + srcSelected.size },
    }));
  }
  ...
  const poolFiltered = srcSearch.trim()
    ? approvedPool.filter((p) => p.question.question.toLowerCase().includes(srcSearch.toLowerCase()))
    : approvedPool;
          ...
          <div className="bg-white rounded-2xl border border-black/10 shadow-xl w-full max-w-sm mx-4 px-6 py-5 space-y-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Bạn có chắc muốn xóa <span className="font-medium text-gray-800">{confirmDelete.name}</span>? Hành động này không thể hoàn tác.
              </p>
            </div>
      ...
      <div className="grid grid-cols-[260px_1fr] gap-4 items-start">
            ...
            >
              ...
              >
                ...
                <div className="flex gap-1.5 mt-1 flex-wrap">
                  <StatBadge label="câu" value={stats[d.id]?.questions ?? 0} />
                  <StatBadge label="approved" value={stats[d.id]?.approved ?? 0} />
                </div>
              </div>
              {/* Delete button */}
              <button
        ...
        {!activeId ? (
          ...
          <div className="space-y-3">
            ...
            {/* Import panel — từ pool câu đã duyệt */}
            {showImport && (
              <div className="bg-white rounded-xl border border-black/8 px-5 py-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{approvedPool.length} câu approved từ các dataset khác</p>
                  </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => {
                      const ids = poolFiltered.map((p) => p.question.id);
                ...
                <div className="max-h-64 overflow-y-auto border border-black/6 rounded-lg divide-y divide-black/5">
                  {poolFiltered.length === 0 && (
                    <p className="text-sm text-gray-400 p-6 text-center">
                      {approvedPool.length === 0 ? "Đang tải..." : "Không tìm thấy câu nào"}
                    </p>
                  )}
                  {poolFiltered.map(({ question: q, datasetName }) => (
                    <label key={q.id} className={cn("flex items-start gap-3 px-3 py-2.5 cursor-pointer hover:bg-gray-50 transition-colors", srcSelected.has(q.id) && "bg-emerald-50/60")}>
                      ...
                      <span className="text-xs text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded shrink-0 self-center">{datasetName}</span>
                    </label>
                  ))}
                </div>
              ...
              {qLoading ? (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-black/5">
                    {filtered.length === 0 && (
                      <tr><td colSpan={4} className="py-12 text-center text-sm text-gray-400">Chưa có câu hỏi nào</td></tr>
                    )}
                    {filtered.map((q) => (
                      <tr key={q.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-3 py-3 text-center">
                          {q.is_approved
                            ? <span className="inline-flex items-center gap-1 text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full"><CheckCircle2 size={11} /> Approved</span>
                            : <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">Draft</span>
                          }
                        </td>
                        <td className="px-3 py-3 text-center">
                          <button onClick={() => removeQuestion(q.id)} title="Xóa khỏi dataset"
